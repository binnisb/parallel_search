...
\hrulefill \\[0.4cm]
{ \huge \bfseries Final project, parallel search}\\[0.4cm]
\hrulefill \\[1.5cm]
...
\lstinputlisting[language=C]{../data/example_data.txt}  
...
But if we look at the whole thing and let $P$ be the time percent of the search in the sequential 
code then from Amdahl's law we expect the speedup on $N$ threads to be:
